...
当前测试聚焦确定性的环境行为、新增设备模型、Agent 工具执行、Agent 控制器主路径和浏览器展示网关，不启动 uvicorn 服务，也不连接 LLM 服务。
环境 HTTP 边界使用 FastAPI `TestClient` 在进程内验证，避免测试依赖外部端口。

Agent 控制器测试使用假流式模型客户端按顺序返回 ReAct 文本，用于验证直接回答、工具调用后回答、空输出继续推理、流式工具调用和模型异常兜底。
浏览器展示网关测试使用 FastAPI `TestClient` 验证静态页面、手动设备控制、SSE Agent 事件和环境状态同步。
