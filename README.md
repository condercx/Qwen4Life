...
当前 Agent 默认内置 2 个基础工具；启用长期记忆后会额外注册 3 个记忆管理工具：
...
`SimpleSmartHomeAgent` 支持可选长期记忆，默认关闭。长期记忆只负责检索和保存上下文，不会替代 ReAct controller 生成最终答案，也不会覆盖工具返回的实时设备状态。当前设备状态和控制结果始终以工具 `Observation` 为准。

当前默认实现使用本地 Ollama 的 `bge-m3` embedding 模型把文本转成向量，并把记忆写入本地 ChromaDB。记忆不是存进模型权重里，而是存放在项目相对路径：

```text
.agent_memory/chroma
```

该目录是本地生成的测试/运行数据，可以删除后重建，不应提交到 git。`.gitignore` 已忽略 `.agent_memory/`、`chroma_db/` 和 `chroma_bench_kg/`。
...
- 检索时会合并向量检索、BM25 关键词检索、轻量 KG 实体/关系匹配和 query expansion。
- query expansion 当前是本地轻量规则扩展，不额外调用 LLM。
- 成功回答后，Agent 会额外调用一次 LLM 做长期记忆保存决策，输出结构化 JSON。
- 只有 Agent 判断本轮包含长期有价值的信息时才保存，包括用户偏好、设备别名、习惯、家庭规则和历史约定。
- 普通寒暄、一次性设备操作、实时设备状态、fallback、异常和空回答不会写入长期记忆。
- 实际保存的是 Agent 提炼出的简洁记忆文本，不保存 `Thought`、`Action`、`Observation` 或 raw messages。

启用长期记忆后，Agent 还会注册 3 个记忆管理工具：
