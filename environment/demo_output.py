...
def initialize_log_file(log_path: Path, chosen_demos: list[str]) -> None:
    ...
    log_path.write_text(f"{json.dumps(header, ensure_ascii=False, indent=2)}\n\n", encoding="utf-8")


def append_log(log_path: Path, demo_name: str, stage: str, payload: dict[str, Any] | list[dict[str, Any]]) -> None:
    """向日志文件追加一条结构化记录。"""
...
def print_observation_summary(label: str, session_id: str, observation: dict[str, Any]) -> None:
    """打印状态观测摘要。"""
...
def print_step_summary(request: dict[str, Any], response: dict[str, Any]) -> None:
    """打印单次动作执行摘要。"""
...
def print_events_summary(events: list[dict[str, Any]]) -> None:
    """打印事件汇总。"""
